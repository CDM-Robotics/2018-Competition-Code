...
public class MotionProfileManager {
     ...
     * motion profile.
     */
    private ArrayList<TalonSRX> _talons;
    /**
    ...
    class PeriodicRunnable implements java.lang.Runnable {
        public void run() {
            for(int i=0; i < _talons.size(); i++) {
                _talons.get(i).processMotionProfileBuffer();
            }
        }
    }
     ...
     */
    public MotionProfileManager(ArrayList<TalonSRX> talons) {
        _talons = talons;
		/*
		 ...
		 */
		for(int i=0; i < _talons.size(); i++) {
            _talons.get(i).changeMotionControlFramePeriod(5);
        }
    ...
    public void reset() {
		 ...
		 */
		for (int i=0; i < _talons.size(); i++) {
            _talons.get(i).clearMotionProfileTrajectories();
        }
    ...
    public void control() {
		...
		for (int i=0; i < _talons.size(); i++) {
            ...
            if (_talons.get(i).getControlMode() != ControlMode.MotionProfile) {
                /*
    ...
    private void startFilling(double[][] profile, int totalCnt) {
		...
		/* did we get an underrun condition since last time we checked ? */
			 ...
			 */
                _talons.get(i).clearMotionProfileHasUnderrun(0);
            }
		/*
		...
		/* set the base trajectory period to zero, use the individual trajectory period below */
            _talons.get(i).setControlFramePeriod(Constants.kBaseTrajPeriodMs, Constants.kTimeoutMs);
		...
		/* This is fast since it's just into our TOP buffer */
			...
			/* for each point, fill our structure and pass it to API */
                ...
                _talons.get(i).pushMotionProfileTrajectory(point);
            }
        }
    }
    /**
